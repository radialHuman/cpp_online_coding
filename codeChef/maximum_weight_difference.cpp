...
int main()
{
    int trails,total,for_son,diff,n;
    std::vector<int> input, output1,output2;
    std::cin >> trails;
    for (auto i = 0; i<trails; i++)
    {
        std::cin >> total >> for_son;
        ...
        std::vector<int> output1(input.begin(),input.begin()+for_son);
        std::vector<int> output2(input.begin()+for_son,input.end());
        std::cout << std::accumulate(output2.begin(), output2.end(), 0) - std::accumulate(output1.begin(), output1.end(), 0);
    }
}
